...
select sum(salary) as SUM_of_Salary, max(salary) as Highest_Salary, min(salary) as Lowest_Salary, avg(salary) as Average_Salary from employee;
...
select p.pnumber, d.dnumber, e.lname, e.address, e.bdate, from employee e, project p, dept_locations d where d.dlocation = 'Stafford' and e.dno=p.dnum and e.dno=d.dnumber;
select p.pnumber,d.dnumber,e.lname,e.address,e.bdate from employee e,project p, dept_locations d where d.dlocation='Stafford' and e.dno=p.dnum and e.dno=d.dnumber;


select fname from employee where salary>avg(salary);

insert into project (pname, pnumber, plocation, dnum) ('ProjectABC', '9', 'Kolkata', '9' );

insert into department values ('NEWdpt', '9', '282828289', '2002-11-11');
